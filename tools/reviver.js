...
// ⚠ 2026-08-27: schema ฉบับเดิม (ซ้อน 4 ชั้น + คำอธิบายไทยยาวทุก field) โดน harness ปฏิเสธ
...
const buildPrompt = (firm) => [
  ...
  ``,
  `═══ ⚠ ถ้าโค้ดนี้เขียนโดย AI/ซีซั่นเดียวกับที่รีวิว (ปกติของที่นี่) ═══`,
...
// ⚠ บั๊กที่เจอตอนรันจริงครั้งแรก (SHIP-MONITORING 2026-08-26): เดิมจับคู่ด้วย "ชื่อไฟล์" อย่างเดียว
...
// ⚠ path เต็มกับ path ย่อ = ที่เดียวกัน (reviver #26 บริษัท A · 2026-08-27):
...
//  ⚠ ผมยังไปแก้มันตอน F9 ด้วย = เสียแรงแก้ของที่ไม่มีใครใช้)
...
const scoreAgreement =
  ...
  : nums.length < FIRMS.length ? `⚠ เทียบได้แค่ ${nums.length}/${FIRMS.length} บริษัท` +
...
const byDim = {}
// ⚠ อย่าเดามิติจาก "ตัวอักษรแรก" ของข้อความที่ agent พิมพ์มา (reviver #26 บริษัท A · 2026-08-27):
...
const dimensionComparison = Object.keys(byDim).sort().map(k => {
  ...
  const flag =
    ...
    : ns.length === 1 ? `⚠ มีบริษัทเดียวให้คะแนนมิตินี้ (${rows[0].firm}) — **เทียบไม่ได้ ไม่ใช่ "ตรงกัน"**`
...
// ⚠ บั๊กรอบสอง (SHIP-MONITORING 2026-08-26): เดิมจับที่ "ตำแหน่ง" อย่างเดียว → over-report (20 แถวจาก
...
// ⚠ บั๊กรอบสาม (พบโดย reviver เอง บน SHIP-MONITORING 2026-08-26): เวอร์ชันก่อนตัดคำด้วย
...
const matcherHealth =
  comparedPairs === 0 ? '⚠ ไม่ได้เทียบสักคู่ — ไม่มี findings หรือไม่มี cleared เลย (ไม่ใช่ "ไม่มีข้อขัดแย้ง")'
...
const intentAgreement =
  ...
  : new Set(statedIntents.map(i => norm(i.intent))).size !== 1
      ? '⚠ เข้าใจเจตนาไม่ตรงกัน — Coddy ต้องตัดสินว่าเจตนาจริงคืออะไรก่อนชั่งน้ำหนัก findings'
  : statedIntents.length < FIRMS.length
      ? `⚠ ${statedIntents.length}/${FIRMS.length} บริษัทที่ระบุเจตนาเข้าใจตรงกัน — อีก ${FIRMS.length - statedIntents.length} ไม่ระบุ (= ไม่รู้ ไม่ใช่เห็นด้วย)`
...
return {
  ...
  next: (reports.length < 2 ? [
    ...
    ...(reports.length < FIRMS.length
      ? [`⚠ หมายเหตุ: ได้ผล ${reports.length}/${FIRMS.length} บริษัท (ขาด ${deadFirms.join(', ')}) — การตรวจทานอ่อนกว่าที่ออกแบบไว้ ต้องเขียนข้อนี้ลงในรายงานถึง Nick ด้วย`]
      : []),
  ]).join('\n'),
}
