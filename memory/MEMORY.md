...
- ⚠️ **`Path.write_text()` บน Windows แปลง `
` → `
`** — เขียนไฟล์ซอร์สด้วย python ต้องใช้ `open(..., newline="")` เสมอ (ตระกูลเดียวกับบั๊ก BOM ของ v1.044 · เทส `test_no_source_file_carries_a_bom_or_cr` จับได้)
**v1.044 ที่ต้องจำ:**
