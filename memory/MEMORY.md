...
**สถานะ:** v1.046 (2026-09-22 · build ล่าสุดที่ปล่อยจริง = v1.045)
...
- 🔗 **rename ของ v1.045 เกือบทำ manifest หาย** — `rename_photos_for_folder` เปลี่ยนชื่อ*ทุกไฟล์* → `job.json` จะกลายเป็น `<job>_003.json` · ตอนนี้ **ข้ามไฟล์ที่ไม่ใช่รูป** + **คืน map ชื่อเก่า→ใหม่** (เก็บที่ `JobAssignment.photo_renames` · merge ที่เปลี่ยนชื่อซ้ำก็อัปเดต map ให้) → `photos[].file` ตามทันเสมอ ไม่งั้นแท็ก Before/After หลุดแบบไม่มีใครรู้
- 📅 **นิกตัดสิน 2026-09-22: กฎคลัง (เลขวันห้ามซ้ำ + ยุบเข้าเดือนหลัก) ชนะ `work_date` จากมือถือ** → วันที่โฟลเดอร์ถูกเลื่อนได้ · เก็บความจริงไว้: `work_date` ไม่ถูกแก้ + เพิ่มบล็อก `filed` (folder/folder_date/date_shifted/merged) · **ข้อยกเว้นเดียว**: งานชื่อเดียวกัน+วันเดียวกันที่มีโฟลเดอร์อยู่แล้ว = **merge เข้าไป** ไม่ใช่เลื่อนไปวันว่าง (ไม่งั้นงานเดียวกินสองโฟลเดอร์สองวัน) — กลับคำได้ที่ branch `same_job_today`
