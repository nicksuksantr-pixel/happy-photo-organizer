...
| 8.4 | **Run the thing.** Tests, the linter, `node --check`, the actual button — not the library call the button is supposed to make | R2 #8: a live verification called `download()` directly while the button had been broken for a release |
