...
> ⚙ **#16.1 — `Workflow` + "ultracode" are INSIDE the cap (2026-09-25):** one launch ran **54 agents** because the count was in the **data, not the script** (`parallel(findings.map(...))` = 1 agent per finding, while `meta.description` honestly said "five lenses"). **"ultracode" lifts the TOKEN limit only**, and typing the word re-arms it even when only asking about it. Every workflow script needs a hand-counted `// MAX_AGENTS: <n>`, and every `parallel()`/`pipeline()` must fan out over a literal array, `Array.from({length:N})` or an explicit `.slice(0, N)` — a bare `.map()` over runtime data is forbidden; >5 → STOP + ask Nick. Enforced by a `PreToolUse` hook — tracked `.claude/settings.json` runs `tools/workflow-agent-cap.mjs`, so **cloud is gated too**. ⚠ It needs `node` on PATH and **fails OPEN if it cannot run**, so this line is still the real rule; the hook only catches you forgetting it.
